...
│   ├── {group}/
│   │   ├── context/    ← group-level shared glossary and API contracts
│   │   ├── adr/        ← group-level architecture decisions
│   │   └── {project}/
│   │       ├── context/
│   │       ├── adr/
...
│   │       └── reviews/
├── skills/             ← reusable agent skills (grill-plan, review, improve-architecture, handoff)
├── templates/          ← format templates for ADRs, plans, context files, reviews
...
| `/gg [partial name]` | Execute the active plan — runs tasks, verifies each, archives on completion |
