...
- `.grimoire/` ephemeral, per worktree, gitignored, always available. Holds `plan.md`, `review.md`, `pr.md`, `brief.md`, `message.md`, `standup.md` and `handoffs/`. Per worktree, so `plan.md` always means the current branch. Everything but `handoffs/` is overwritten each run, so nothing there is a durable record.
