...
- a plan task named it in `verify:`. Read `.scribe/plan.md` when it exists and match against the `verify:` lines.
...
UNREQUESTED TESTS
  path/to/new.test.ts  4 cases, no verify: line in .scribe/plan.md names them
